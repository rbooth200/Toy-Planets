...
// Build with:
//  g++ -O3 -shared  block_tri_solve.cpp -I/usr/include/eigen3/ -Wall -fPIC -o block_tri_solve.so -DEIGEN_NO_DEBUG
...
BlockTriDiagSolverBase *create_BlockTriDiagSolver(int num_blocks,
                                                  int block_size)
{
  switch (block_size)
  {
  case 1:
    return new BlockTriDiagSolver<1>(num_blocks);
  case 2:
    return new BlockTriDiagSolver<2>(num_blocks);
  case 3:
    return new BlockTriDiagSolver<3>(num_blocks);
  case 4:
    return new BlockTriDiagSolver<4>(num_blocks);
  case 5:
    return new BlockTriDiagSolver<5>(num_blocks);
  case 6:
    return new BlockTriDiagSolver<6>(num_blocks);
  case 7:
    return new BlockTriDiagSolver<7>(num_blocks);
  case 8:
    return new BlockTriDiagSolver<8>(num_blocks);
  case 9:
    return new BlockTriDiagSolver<9>(num_blocks);
  case 10:
    return new BlockTriDiagSolver<10>(num_blocks);
  default:
    return new BlockTriDiagSolver<Eigen::Dynamic>(num_blocks, block_size); 
  }
}
...
extern "C"
{
  ...
  void *factor_block_tridiag(int num_blocks, int block_size,
                             double *l, double *d, double *u)
  {

    BlockTriDiagSolverBase *tridiag =
        create_BlockTriDiagSolver(num_blocks, block_size);

    tridiag->factor_matrix(l, d, u);

    return tridiag;
  }

  void solve_block_tridiag(void *_tridiag, double *rhs, double *x)
  {
    BlockTriDiagSolverBase *tridiag =
        reinterpret_cast<BlockTriDiagSolverBase *>(_tridiag);

    tridiag->solve(rhs, x);
  }

  void delete_tridiag(void *tridiag)
  {
    delete reinterpret_cast<BlockTriDiagSolverBase *>(tridiag);
  }

  void block_tridiag_solve(int num_blocks, int block_size,
                           double *l, double *d, double *u,
                           double *rhs, double *x)
  {
    std::unique_ptr<BlockTriDiagSolverBase>
        tridiag(create_BlockTriDiagSolver(num_blocks, block_size));
